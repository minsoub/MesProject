...
function readDB(){
	var urlname='http://localhost/RES2/treetable/read_collection.php?collection=master';
	get_json_from_mongodb(urlname);	  // make all ts for each excelRange 
	BOM_master=from_ajax_data;
//	console.log(BOM_master);
	urlname='http://localhost/RES2/treetable/read_collection.php?collection=transaction';
	get_json_from_mongodb(urlname);	  // make all ts for each excelRange 
...
function populateBOM_TreeTable(){
	...
	for (var i = 0; i < BOM_master.length; i++){
		...
		$(cur_tr).after('<tr data-depth="0" class="tbcollapse level0">'
		...
		for (var j=0;j<level0.sub.length;j++){
            ...
            $(cur_tr).after('<tr data-depth="1" data-l_id="'+level0.l_id+'" class="tbexpand level1">'
			...
			if (BOM_transaction[loc].BOM_id==(level0.l_id+'@'+level1.m_id)){
                 $(cur_tr).after('<tr data-depth="2" class="tbcollapse level2 tr_inventory">'
                        		+'     <td>재고량</td>'
                        		+'      <td></td><td></td><td></td>'
                        		+'     <td><span class="inventory">'+BOM_transaction[loc].inventory+'</span></td>'
                        		+' </tr>');
				cur_tr=$(cur_tr).next();
                $(cur_tr).after(' <tr data-depth="2" class="tbcollapse level2 tr_input">'
                ...
                $(cur_tr).after(' <tr data-depth="2" class="tbcollapse level2 tr_output">'
...
function expand_collapse_TreeTable(){
	var tr_expands=$('#BOM_table').find('tbody>tr.tbexpand');
